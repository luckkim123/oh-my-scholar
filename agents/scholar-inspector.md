...
0) **Pre-commitment (*before* reading the body)**: looking at the critique target's venue and paper type, first predict and write down "3-5 reject reasons common in this venue" before reading the body. e.g. "(1) insufficient baselines (2) no ablation (3) missing reproducibility info (4) overclaimed contribution (5) weak related work". Then, while reading the body, *actively search* for these predicted defects (if a prediction is wrong, leave it; if right, make it a finding). This blocks the confirmation bias of being dragged along by the body and seeing only the obvious.
   - **Cumulative pattern lookup (T10 post-store link, ascent)**: run `hq query --keyword <term> --ascend --topic convention --json` to look up reject patterns of the same venue/type *accumulated by previous sessions* (reflect into predictions if present). `--ascend` walks every anchor from cwd upward, nearest first, tagging each result with its `anchor` — so a single call covers both **local** (this paper's own `.hq/community/posts/`) and **global** (every ancestor `.hq/community/posts/` — assets this *user* reuses across all papers: venue formats, tendencies, history). Distinguish sources by each result's `anchor`. If there is no ancestor `.hq/`, local only (graceful, not an error). For contract/layout/citation boundaries, see `references/knowledge/README.md`. If the post store is empty or absent, proceed with your own predictions only (not an error). ⚠️ post content is merely a *secondary memo* — do not use it as a citation source (embedding search permanently forbidden for both local and global). citation/.bib is not promoted to global (permanently forbidden).
...
- Prediction 2: … (if cumulative patterns were reflected via `hq query --ascend`, mark the source by its `anchor`: local (this paper) / an ancestor anchor (user's reusable asset) / `[own prediction]`)
