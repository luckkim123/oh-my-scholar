...
- If a note already exists for this paper → read `.hq/community/reading/<citekey>.md` directly instead of re-dispatching
</Do_Not_Use_When>
...
- ⚠️ **NOT CITABLE — secondary memo only** — `.hq/community/reading/<citekey>.md` is never a `.bib` source. The only door into the bibliography stays scholar-research → human-confirmed `.bib` (invariant 2). `<citekey>` here is a *filename convention* (`<firstauthor><year>-<short-slug>`), explicitly NOT a BibTeX key.
...
3. **Single dispatch**: `Task(subagent_type="oh-my-scholar:scholar-researcher", mode="deep-read", ...)`:
   - Input: the resolved paper (path/id/text), the identity verdict from step 2, any project context the caller wants the note related to (optional — `.hq/work/scholar/<slug>/outline/` / `methodology/` paths if relevant)
...
4. **The calling session writes** the note to `.hq/community/reading/<citekey>.md` (the dispatched agent stays read-only — same writer-identity carve-out as `reviews-log.md`/`research-log.md`). The note's first line is the mandatory header: `> NOT CITABLE — secondary memo. A .bib entry may only be created via scholar-research verification.`
5. **research-log append** (`references/output-layout.md` §2.4, context `read`): one dated entry `## YYYY-MM-DD — read` in `.hq/work/scholar/<slug>/research-log.md` summarizing what was read and why (create-if-absent, append-only). Skipped when there is no active paper slug to attach it to.
...
<Output>
- `.hq/community/reading/<citekey>.md` — the reading note (Paper identity / Claims / Method / Evidence / Limitations / Relation to my work if applicable / Open questions), NOT CITABLE header as its first line
