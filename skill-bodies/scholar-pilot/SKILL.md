...
10. **post capture (auto-specialization — the more you use it, the more it adapts to this project)**: patterns that inspect/verify *discovered* this session and that are reusable are **automatically posted** via `hq post --topic <category>` to the **target project's `.hq/community/posts/`** (the project workspace, not the plugin; tracked in that project, store-spec §3/§5) (no approval needed — light channel). This is the data that the next session's inspector's pre-commitment `hq query --ascend --topic <category>` reads — with writing and reading forming a closed loop, the more you use the harness, the more it specializes to this venue/this paper project. (Being a workspace, it does not pollute plugin distributions·other projects, and it isn't blown away by a marketplace update.)
    - **What to load**: ① per-venue recurring reject patterns → `hq post --topic convention --subject <venue>-reject-patterns` ② the rationale for the baseline·comparison group·structure chosen this time → `hq post --topic decision --subject <slug>` ③ discovered external-resource pointers → `hq post --topic reference`. Only what the inspector/verifier actually saw — no speculative loading.
    - **conclusion + rationale together (no label-only)**: each entry should not record only the *conclusion (label)* but also the *concrete rationale* that supports it (which case·control group·which paper-slug/section to re-read = internal pointer) on the same line/item. Instead of "X is the stage axis", write "X juxtaposes two independent contributions across stages — `<slug>` §toc". If you record only the label, the next session has to re-read the original. ⚠️ The internal pointer is *re-visit navigation*, not a `.bib` citation (see citation-safe below·`references/knowledge/README.md`). Recommended, not a gate (light channel).
    - **post format**: each entry is its own `hq post --category finding --topic <category> --subject <key> ...`, body = free prose. If the same pattern already exists (check with `hq query --subject <key>` first), don't post a duplicate — a genuine update goes through `--supersedes <id>` rather than a fresh unrelated post. `hq` owns the frontmatter shape — no manual schema to compose (see `references/knowledge/README.md`).
    - **light-channel evidence signal (append-time, #24)**: every posted entry states its evidence — an internal pointer (`<slug> §…`) or a verbatim quote. An entry with neither is **still posted** (no reject gate), but it is posted at `--confidence low` with a `(evidence: none — add a pointer before confidence can rise)` marker in the body. Evidence-less re-observation never raises confidence — only a pointer or quote lifts a follow-up post past `low` (`references/knowledge/README.md` § confidence).
    - **global promotion candidate hint (terminal only)**: at pipeline shutdown, if among what accumulated locally in `.hq/community/posts/` this time there are assets *reusable for the next paper too* (tendencies·venue formats·reusable decisions·history), recommend to the user "Shall we promote this to the ancestor anchor (global)?" — actual promotion goes through `scholar-learn`'s local→global path (human gate). ⚠️ citation/.bib is permanently forbidden from global promotion. No automatic promotion.
    - ⚠️ **citation-safe (mandatory — violation collapses OMS identity)**: the post store is **secondary memo only**. The *content* of citations·claims·numbers is never loaded (only reject reasons·format rules that aid prediction). The .bib is updated only from scholar-research-verified primary sources — post excerpts are not pulled in as citations. Both loading·querying use **deterministic text only, no embeddings**. For the contract·boundary see `references/knowledge/README.md`.
    - **automatic but non-destructive**: each observation is its own new post (an existing post is never rewritten by this step), and if there's nothing to load it just passes (empty session OK). `hq post` creates `.hq/community/posts/` lazily on the first write. If the user passes `--no-post`, skip.
    - **research-log entry (append-only project narrative memory, `references/output-layout.md` §2.4)**: append one dated entry to `.hq/work/scholar/<slug>/research-log.md` (create-if-absent) — heading `## YYYY-MM-DD — pilot`, then free-prose bullets covering `tried / decided / dropped — and why` for this run (stages executed, GATE outcomes, major decisions, dropped directions). Written by the calling session, never a dispatched agent — same writer identity as the post writes above. ⚠️ secondary memo only: a citation key in a research-log entry is never citation authority and this file is **never a `.bib` source** (invariant 2). If the user passes `--no-log`, skip.
