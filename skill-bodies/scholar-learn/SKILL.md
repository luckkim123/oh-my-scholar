...
in `.hq/config/scholar/learned.md` (e.g. "IROS papers always have an Ablation section — repeated 3 times") and judges which
...
<Use_When>
- During operation, when enough observations have accumulated in `.hq/config/scholar/learned.md` to judge "should we now solidify these into venue defaults?"
...
<Do_Not_Use_When>
- If `.hq/config/scholar/learned.md` is empty → there are no observations to promote yet. inspect/verify fill it as you operate.
- If it's merely a light pattern, preference, or decision memo and not an enforced default → don't promote; leave it to auto-accumulate in `.hq/community/posts/` (no gate needed, recovered via `hq query` next session). In particular, `topic: pattern` (preferences) is permanently light — never a promotion target. Not every observation becomes a default.
...
- ⚠️ **Respect the 2-channel separation** — only the *heavy channel* (defaults: learned.md → promote → venues.md) is the target of this skill and passes the gate. The *light channel* (patterns/preferences/decisions: `.hq/community/posts/` auto-append) needs no gate — read only, don't touch. `topic: pattern` (preferences) is permanently light.
- ⚠️ **Local→global post elevation (separate path, storage-location axis)** — separate from `scope: global` above being a *scope-of-application* label in venues.md, there is a path that elevates light-channel assets by **physical storage location** from the local `.hq/community/posts/` to the *parent folder's `.hq/community/posts/`* (global level, discovered by ascent). At paper close, candidates that "this asset is reusable for the next paper too" (preferences, venue formats, reuse decisions, history) are posted **after human approval** to the parent anchor with the same `subject:`. Criteria:
  - Targets: among `topic: pattern` (preferences), `convention` (venue formats), `decision` (reuse decisions), `history` (paper records), only *paper-agnostic reusable* assets. This paper's unique rejects/gaps stay local.
  - ⚠️ **citation/.bib are permanently forbidden from global elevation** (whether heavy or light, `learning-protocol.md` §6.F·§1.4).
  - Human gate required — no automatic elevation. If the parent `.hq/` anchor does not exist, advise "run `hq post` once in the parent folder to create its anchor" (do not arbitrarily create one in the parent or home).
  - This is a different lane from venues.md default promotion (heavy) — it's merely a *location* move of light assets, not making them an enforced default. `topic: pattern` remains permanently light even when elevated to global (not enforced).
...
- The canonical reference for the learning channels, promotion criteria, and specificity formula is `references/learning-protocol.md`; the venue schema is `references/venues.md`; the post-store convention SSOT is `references/knowledge/README.md`.
</Execution_Policy>

<Steps>
1. **Confirm SSOT and prerequisites**: check that the working root and `.hq/config/scholar/learned.md` exist. If empty, stop and advise "there are no observations to promote — inspect/verify fill it as you operate". Read the following:
   - `.hq/config/scholar/learned.md` — observations awaiting promotion (the input to this skill)
   - `references/venues.md` — existing venue defaults to evolve (an *evolve*, not a blind replace)
   - `hq query --topic convention --ascend --json` — light channel. Check whether high-confidence signals have accumulated (read only)
   - `references/learning-protocol.md` — 2-channel, promotion criteria, specificity formula (canonical)
2. **Classify observations (2-channel discrimination)**: split each observation in learned.md into (a) venue default promotion candidates (heavy channel — gate target) vs (b) pattern/preference memos (light channel — leave in the post store, no gate needed). Not every observation becomes a default — apply the learning-protocol.md channel criteria. Citation/.bib types are excluded immediately (§6.F).
...
4. **Delegate to agent (promotion judgment — read-only)** — single delegation to scholar-inspector. Fresh subagent to prevent controller-context pollution. Since it's one careful synthesis, **no parallel inspectors**:
   ...
   Task(
     ...
     prompt="""
     Role: scholar-learn promotion judgment. Read the .hq SSOT below and, among the learned.md observations,
     ...
     Inputs (to read):
     - .hq/config/scholar/learned.md                # observations awaiting promotion (incl. scope, evidence, counterexamples, user_stated)
     - references/venues.md           # existing defaults to evolve (evolve, not replace)
     - hq query --topic convention --ascend --json   # light-channel confidence signals (read only)
...
5. **Commit approved items (only after passing the gate)**: only defaults the human approved are written to disk by this skill.
   ...
   - `references/venues.md` (or project `.hq/config/scholar/venues/<key>.yaml`) — add/change approved defaults, source observation id in `learned_refs[]`, mark the relevant item `learned` in `origins`, recompute `specificity` per scope. (Re-confirm schema conformance.)
   ...
   - `.hq/config/scholar/learned.md` — mark promoted observations "promoted → venues.md (date)", keep held ones as candidates (re-evaluated at the next learn).
6. **Post store → reference-card anchoring (a third lane — distinct from venue-default promotion above and the local→global post elevation bullet in Execution_Policy)**: promote a mature *global*-post cluster into a `references/` card, making the 2026-06 hand-done pattern (an external survey anchored into 5 oms files) a repeatable step. Runs independently of Steps 1-5 — no `learned.md`/`venues.md` involvement, a separate target.
   - **Trigger**: a *global*-post cluster (the parent-ascent `.hq/community/posts/` level, `references/learning-protocol.md` §1.4) that is paper-agnostic and harness-relevant (informs how oms itself should work, not this paper's content) — `confidence: high` with `sightings ≥ 3` — or the user explicitly requests it regardless of confidence/sightings.
   - **Verb**: propose a `references/` card draft (new file) or an existing-card update that **anchors, never copies wholesale** — pointers into the source post(s) (`hq query --post-id <id>`) plus `file:line` anchors into the affected oms surfaces (skills/agents/other reference cards), following the "no duplicate embedding — reference the SSOT" card discipline (`references/writing-craft.md:3`). The proposal states what changed and why; it does not paste post prose verbatim into the card.
