...
- If it's merely a light pattern, preference, or decision memo and not an enforced default → don't promote; leave it to auto-accumulate in `.hq/community/wiki/` (no gate needed, recovered via grep next session). In particular, `wiki/pattern/` (preferences) is permanently light — never a promotion target. Not every observation becomes a default.
...
- ⚠️ **Respect the 2-channel separation** — only the *heavy channel* (defaults: learned.md → promote → venues.md) is the target of this skill and passes the gate. The *light channel* (patterns/preferences/decisions: `.hq/community/wiki/*.md` auto-append) needs no gate — read only, don't touch. `wiki/pattern/` (preferences) is permanently light.
- ⚠️ **Local→global wiki elevation (separate path, storage-location axis)** — separate from `scope: global` above being a *scope-of-application* label in venues.md, there is a path that elevates light-channel assets by **physical storage location** from the local `.hq/community/wiki/` to the *parent folder's `.hq/community/wiki/`* (global level, discovered by ascent). At paper close, candidates that "this asset is reusable for the next paper too" (preferences, venue formats, reuse decisions, history) are copied **after human approval** into the parent `.hq/community/wiki/<category>/`. Criteria:
  ...
  - Human gate required — no automatic elevation. If the parent `.hq/` does not exist, advise "init in the parent folder or place a `.hq/community/wiki/` there" (do not arbitrarily create one in the parent or home).
...
1. **Confirm SSOT and prerequisites**: check that the working root and `.hq/config/scholar/learned.md` exist. If empty, stop and advise "there are no observations to promote — inspect/verify fill it as you operate". Read the following:
   ...
   - `.hq/community/wiki/convention/*.md` — light channel. grep whether high-confidence signals have accumulated (read only)
...
4. **Delegate to agent (promotion judgment — read-only)** — single delegation to scholar-inspector. Fresh subagent to prevent controller-context pollution. Since it's one careful synthesis, **no parallel inspectors**:
   ...
   Task(
     ...
     - .hq/community/wiki/convention/*.md      # light-channel confidence signals (read only)
...
6. **Wiki → reference-card anchoring (a third lane — distinct from venue-default promotion above and the local→global wiki elevation bullet in Execution_Policy)**: promote a mature *global*-wiki cluster into a `references/` card, making the 2026-06 hand-done pattern (an external survey anchored into 5 oms files) a repeatable step. Runs independently of Steps 1-5 — no `learned.md`/`venues.md` involvement, a separate target.
   - **Trigger**: a *global*-wiki cluster (the parent-ascent `.hq/community/wiki/` level, `references/learning-protocol.md` §1.4) that is paper-agnostic and harness-relevant (informs how oms itself should work, not this paper's content) — `confidence: high` with `sightings ≥ 3` — or the user explicitly requests it regardless of confidence/sightings.
