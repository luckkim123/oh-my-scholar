...
3. Single delegation via `Task(subagent_type="oh-my-scholar:scholar-drafter", ...)`:
   ...
   - Instructions: one section at a time serially, every \cite only to a verified .bib, flag unverified citations, snapshot to `.oms/<slug>/versions/` before large edits.
...
<Output>
List of .tex/.bib files the drafter wrote (project source folder) + snapshot location (`.oms/<slug>/versions/`) + list requiring human confirmation (unverified citations, etc.) + "ready to hand off to scholar-verify" (explicitly no self-approve).
