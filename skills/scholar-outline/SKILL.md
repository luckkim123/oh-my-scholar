...
- **`--consensus`**: planner의 `<Consensus_RALPLAN_DR_Protocol>`을 발동해 story arc Options≥2를 거쳐 최종 1개로 수렴. 4-agent **순차** 파이프라인. `plan.md`(결정 과정) + `06_outline.md`(결정 결과) 2분리 산출.
...
**`--consensus` 모드일 때 추가**: `plan.md`(RALPLAN-DR+ADR — 결정 과정) 저장 위치 + 재리뷰 회차(N/5) + consensus 도달 여부("도달" 또는 "not reached — 잔여 finding 목록"). plan.md와 06_outline.md는 *분리된 두 파일*이다 (결정 과정 ≠ 결정 결과).
</Output>
