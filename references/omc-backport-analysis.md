...
| T10 | wiki accumulation | data lives in the project workspace `.oms/wiki/*.md`(gitignore, OMC `.omc/wiki/` pattern) + deterministic grep as the **default**, `wiki_query(category)` is an abstract function (a future MCP swap point). Only the contract doc is in plugin `references/wiki/README.md`. The reject store is net-new (not a venues.md migration) |
