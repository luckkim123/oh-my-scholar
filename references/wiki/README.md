...
⚠️ **Data accumulates in the workspace (`.hq/community/wiki/`) — not in this plugin repo.** This README is a *contract document*, so it ships with the plugin (`references/wiki/README.md`), but the actual accumulated data is written to **`.hq/community/wiki/`** (tracked *in the calling project*, not this plugin — so it diverges per-project without polluting the plugin/distribution, which is how "specialized to this user / this paper" holds). Same as OMC's `.omc/wiki/` (project-local) pattern.

### ⭐ Two layers — local (this paper) + global (parent folder's `.hq/`, found by ascent)

`.hq/community/wiki/` lives at **two levels**. Both are cwd-relative — **zero absolute paths, env vars, or XDG** (oms philosophy "work-root relative" as-is, no distribution pollution):

```
<parent folder of the papers>/.hq/community/wiki/      ← ⭐ global level — reused by this *user* across all papers
  ...
  history/      *.md   ← ⭐new: my paper history (init references for duplicates and linking)
        ▲  discovery method = ascent (cwd→parent, the single nearest ancestor .hq/, excluding self; like git finding .git)
        │
<paper folder>/.hq/community/wiki/                ← local level — specialized to this paper only (outside slug, accumulates across sessions)
...
- ⚠️ `.hq/community/wiki/` is a *project-wide* accumulation, so it sits **outside** the per-task `.hq/work/scholar/<slug>/` (output-layout) — not bound to a slug, it lives across sessions and tasks.
...
of `.hq/community/wiki/`). `scholar-reviewer` (lens mode) reads it when present through the same 2-tier
...
- **Current implementation (2-layer ascent merge)**:
  ```
  local_hits  = grep(<cwd>/.hq/community/wiki/<category>/, keywords)              # local — this paper
  parent_oms  = ascent(<cwd>): go cwd→parent to the first .hq/ (excluding self)     # like git finding .git
...
- **Graceful degrade on absence**: if either local or global is empty, or there is no ancestor `.hq/`, that side returns an empty list — not an error. The inspector proceeds with only what exists (or with its own prediction only).
